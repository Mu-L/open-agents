...
Approach: Introduce a dedicated message-list layer with memoized row components and a bottom-anchored measured virtualizer for variable-height rows. Move the inline row rendering out of `session-chat-content.tsx`, precompute per-message view data once, and make collapsed assistant messages avoid executing the hidden-group render path. Use virtualization to mount only the viewport window plus overscan, while preserving the current auto-scroll-to-bottom behavior for live streams and the existing expand/collapse UX for assistant tool/reasoning content.
...
- `apps/web/package.json` - add a small measured-row virtualizer dependency for dynamic-height chat rows.
