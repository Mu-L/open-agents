...
- When `claimSandboxProvisioning` loses the CAS race, do not "fall through" to sandbox creation after a short wait; that still allows duplicate sandbox creation under real-world provisioning latency. Return a conflict/in-progress response and let clients reconnect/poll the winner instead.
